...
CREATE TABLE `medicalsituation` (
  `medicalSituation` int(10) unsigned NOT NULL,
  `diseases` varchar(500) DEFAULT NULL,
  `weight` double DEFAULT NULL,
  `measurment` varchar(200) DEFAULT NULL,
  `nutId` int(10) unsigned DEFAULT NULL,
  `customerId` int(10) unsigned DEFAULT NULL,
  PRIMARY KEY (`medicalSituation`),
  KEY `nutId` (`nutId`),
  KEY `customerId` (`customerId`),
  CONSTRAINT `medicalsituation_ibfk_1` FOREIGN KEY (`nutId`) REFERENCES `nutritionist` (`nutId`) ON DELETE SET NULL,
  CONSTRAINT `medicalsituation_ibfk_2` FOREIGN KEY (`customerId`) REFERENCES `customers` (`customerId`) ON DELETE CASCADE
